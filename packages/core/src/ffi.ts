...
const lib = backend.loadLibrary(libPath, {
  ...
  bufferHeight: { args: [FFIType.ptr], returns: FFIType.u32 },
  bufferRealCharSize: {
    args: [FFIType.ptr, FFIType.bool],
    returns: FFIType.u32,
  },
  bufferWidth: { args: [FFIType.ptr], returns: FFIType.u32 },
...
export const api = {
  buffer: {
    bufferCellAttributes(p: Pointer<Buffer>, idx: bigint): number {
      return lib.symbols.bufferCellAttributes(p, idx);
    },
    bufferCellChar(p: Pointer<Buffer>, idx: bigint): number {
      return lib.symbols.bufferCellChar(p, idx);
    },
    bufferCellsLen(p: Pointer<Buffer>): bigint {
    ...
    bufferHeight(p: Pointer<Buffer>): number {
      return lib.symbols.bufferHeight(p);
    },
    ...
    bufferWidth(p: Pointer<Buffer>): number {
      return lib.symbols.bufferWidth(p);
    },
    bufferClear(p: MutablePointer<Buffer>, bg: RGBAInput): void {
      lib.symbols.bufferClear(p, rgbaPtr(bg));
    },
    bufferDrawBox(
      buf: MutablePointer<Buffer>,
      x: number,
      y: number,
      width: number,
      height: number,
  ...
  renderer: {
    ...
    processEvents(p: Pointer<Renderer>): bigint {
    ...
    getCurrentBuffer(p: Pointer<Renderer>): ReadonlyPointer<Buffer> {
      return toPointer(
        lib.symbols.getCurrentBuffer(p)
      ) as ReadonlyPointer<Buffer>;
    },
    getNextBuffer(p: Pointer<Renderer>): MutablePointer<Buffer> {
      return toPointer(lib.symbols.getNextBuffer(p)) as MutablePointer<Buffer>;
    },
    ...
    injectResizeEvent(
      ...
      return lib.symbols.injectResizeEvent(p, width, height);
    },
